...
description: 'Kali ini kita akan melakukan deployment menggunakan tools Deployer ke server ubuntu'
date: '08-05-2023 17:20:00'
tags: ["deployer", "laravel"]
draft: false
---
...
buat directory baru di `/var/www sudo -uwww-data mkdir -p shared`
buat file `.env` di `/var/www sudo -uwww-data touch shared/.env`
...
buat konfigurasi file deploy.php di laravel root directory
...
jalankan perintah `./vendor/bin/dep deploy production`

jalankan perintah `php artisan key:generate` di folder `/var/www/app/current`